	...
	private void configurarBotoes() {
		...
		botaoConfirmar.addActionListener(e -> {
			...
			try {
				...
				Cliente novoCliente = new Cliente(cpf, nome, email, telefone,diaDoNascimento, mesDoNascimento, anoDoNascimento);
				Logger logger = Logger.getLogger(MainFrame.class.getName());
				logger.log(Level.INFO, "Cliente criado com sucesso");
				
				if (rua.isEmpty() == false && rua.isBlank() == false) novoCliente.setRua(rua);
				if (bairro.isEmpty() == false && bairro.isBlank() == false) novoCliente.setBairro(bairro);
				if (cidade.isEmpty() == false && cidade.isBlank() == false) novoCliente.setCidade(cidade);
				if (numeroResidencia != -1) novoCliente.setNumeroResidencia(numeroResidencia);
				if (UF != null) novoCliente.setUF(UF);
